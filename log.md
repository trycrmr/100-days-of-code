...
It turns out the stack I was attempting to deploy was failing because CloudFormation's tag syntax was wrong in the template. The thing is the syntax I expected it to be was passing the YAML validators and was described that way in the documentation. I submitted documentation feedback to update the example YAML snippet for an S3 bucket resource to reflect the proper tagging syntax more precisely. 

Unfortunately, if rollback is enabled the console wasn't depicting the specific reason the S3 bucket was not being created. When I disabled rollback the error message denoting the issue with the tags displayed. This was a super easy fix to implement, but, overall, it's a bummer the debugging process changes with/without rollback enabled, and it's a bummer the bug was from a lack of clarity in the documentation. I'm really glad AWS has a feedback button for their docs to submit issues. I've only had to use it a few times, but it's times like these when I'm glad I'm able to play my part in helping others not bumping into the same issue I just did. 
